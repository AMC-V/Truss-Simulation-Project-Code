...
class GraphicsTools():
    ...
    def node_creation(self, x, y, number_of_node):
        if number_of_node > self.number_of_nodes: # so doesn't exit in list of nodes
            ...
            self.list_of_nodes.append(vec(x, y, 0)) # basically only holds a nodes position
        ...
        self.list_of_force.append(0) # leave a placeholder in the list for the node
        self.list_of_angle.append(0)
        return vec(x, y, 0)
    ...
    def node_creation1(self, x, y, number_of_node, force, angle):
        if number_of_node > self.number_of_nodes: # so doesn't exit in list of nodes
            self.list_of_spheres.append(sphere(pos = vec(x, y, 0))) # create sphere and label
            ...
            if (self.list_of_nodes[number_of_node - 1] == vec(x, y, 0)):                
                ...
                # if force does not exist, then replace the placeholder zeroes with actual arrow
                if self.list_of_force[number_of_node - 1] == 0 & self.list_of_angle[number_of_node - 1] == 0:
                    self.force_creation(number_of_node, force, angle)
...
# set = GraphicsTools()
# set.node_creation(1,1,1) # test case 1 
# time.sleep(2)
# set.node_creation(2,2,1) # test case 2
# time.sleep(2)
# set.node_creation1(2,2,1,100,90) # testing new force, doesnt work well
# time.sleep(2)
# # here prob
# set.node_creation1(4,4,1,100,90) # testing movement of force
# time.sleep(6)
# set.node_creation1(4,4,1,50,0) # seeing what happens 
# time.sleep(2)
# set.node_creation1(2,2,1,50,180)
# time.sleep(2)
# set.node_creation1(2,2,1,50,180)
# time.sleep(2)
# set.node_creation1(-1,1,2,100,270)
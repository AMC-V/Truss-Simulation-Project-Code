...
from envitest import GraphicsTools
...
class MainWindow(QMainWindow): # Class that will create UI, will inhertant all the methods from QMainWindow
    ...
    def createWindow(self):
        ...
        self.genericInput.textChanged.connect(self.onTextChanged)
        ...
        self.addNodeButton.clicked.connect(lambda: onClick(self))
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # endregion
    ...
    def onTextFinal(self,number):
        print(f" {len(self.list_of_widgets)} done {self.genericInput_1.text()}")
        ...
        print(f"{number} this is the one  {hey[0].text()}") # go to the first QLineEdit in the list and grab the text from it
        print(f" actual number {hi[0].text()}")
